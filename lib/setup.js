...
program.command('setupAuthentication <inputfile>')
  ...
  '   NOTE: Each "inputfile" is treated as one group; what this means is that' + os.EOL +
  '   after running this command all specified users will have access (at least via SSH)' + os.EOL +
  '   to all specified devices. Further, if there is a failure in configuring some' + os.EOL +
  '   of the specified devices, the same file can be passed in again and' + os.EOL +
  '   the script will try to configure all failed devices. Once a device is configured' + os.EOL +
  '   successfully, the script marks them in the "inputfile" as "done".' + os.EOL +
  '   To reconfigure a device, remove the keyword "done" from the respective line.')
...
program.command('configureCredentials <username> <host_address>')
  ...
  '   credentials. Credentials are expected to be present in the current' + os.EOL + '' +
  '   directory.')
...
function exec_config_credentials(username, hostname, options) {
  ...
  if (!fs.existsSync(localstatedir)) {
    ...
    var clientkeystate = path.join(clientkeydir, ConfigManager.config.stateMapName);
    try {
      localstate = JSON.parse(fs.readFileSync(clientkeystate));
    } catch (e) {
      // parse exceptions aren't very user friendly, say this instead
      throw new Error("Unable to read required file " + clientkeystate +
      " from the credential directory.");
    }
  }

  // todo: combine existing state and new state
  ...
  var initGlobalState = function (done) {
    if (!fs.existsSync(globalstatedir)) {
      ...
      var statemap = path.join(globalstatedir, ConfigManager.config.stateMapName);
      try {
        if (fs.existsSync(statemap))
          globalstate = JSON.parse(fs.readFileSync(statemap));
      } catch (e) {
        // parse exceptions aren't very user friendly, say this instead
  ...
  var copyClientCredentials = function (done) {
    ...
    exec(getCopyClientCredentialsCmd(clientkeydir + " " + localstatedir),
    function (error, stdout, stderr) {
      if (error)
        done (stderr);
  ...
  var configureMosquitto = function (done) {
    ...
    if (fs.existsSync(mosquittoConfig)) {
      ...
      if (sectionStartIdx === -1) { // iotkit-comm section not found. Add to end.
        ...
        parseInt(addLines[3].split(wsRe)[1]));
      } else {
        console.log("Mosquitto server can now accept secure connections on port " +
        parseInt(configLines[sectionStartIdx + 3].trim().split(wsRe)[1]));
      }
  ...
  async.series([initGlobalState, copyClientCredentials, copyHostAndCACredentials, configureSSHClient,
      ...
      changeLocalStateOwnership, changeGlobalStateOwnership, restartMosquitto, saveState],
    function (e) {
      if (e)
        throw e;
    });
