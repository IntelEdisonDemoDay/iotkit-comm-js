...
function ServiceQuery(source) {
  ...
  // make sure service query does not contain address and port
  if (this.constructor.name === "ServiceQuery" &&
    this.sourceObj.address && this.sourceObj.port) {
    throw new Error("A service query cannot contain both an address and port; use service specification instead.");
  }
